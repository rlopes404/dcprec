...
def sample_function(user_train, user_validation, user_test, owner, usernum, itemnum, batch_size, train_queue, valid_queue, test_queue, SEED, consumed_items):
    def sample_ui():
        if not is_test:
        ...
        while item_ip in User[user]['consume'] or item_ip in User[user]['produce'] or item_ip == item_i or item_ip not in consumed_items:  item_ip = np.random.randint(0, itemnum)
        #while item_ip in User[user]['consume'] or item_ip in User[user]['produce'] or  owner[item_ip] in User[user]['consume_owners'] or item_ip == item_i or item_ip not in consumed_items: item_ip = np.random.randint(0, itemnum)
...
class WarpSampler(object):
    ...
    def __init__(self, user_train, user_validation, user_test, owner, usernum, itemnum, consumed_items, batch_size=10000, n_workers=2):
        ...
        for i in range(n_workers):
            self.threads.append(threading.Thread(target=sample_function, args=(user_train,
                                                                               ...
                                                                               self.valid_queue,
                                                                               self.test_queue,
                                                                               np.random.randint(2e9), consumed_items)))
